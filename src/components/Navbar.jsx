...
const NavButton = ({ title, customFunc, icon, color, dotColor }) => (
  <TooltipComponent content={title} position="BottomCenter">
    <button type='button' onClick={customFunc} style={{ color }} className='relative 
...
const Navbar = () => {
  const { setActiveMenu, isClicked, handleClick,
    screanSize, setScreanSize, currentColor } = useStateContext();

  useEffect(() => {
    const handleResize = () => setScreanSize
      (window.innerWidth);
    ...
    <div className='flex justify-between p-2 md:mx-6 relative'>
      ...
      <div className='flex'>
        ...
        <NavButton title="Chat"
          dotColor="#03C9D7" customFunc={() => handleClick('chat')}
          color={currentColor} icon={<BsChatLeft />} />
        <NavButton title="Notifications"
          dotColor="#03C9D7" customFunc={() => handleClick('notification')}
